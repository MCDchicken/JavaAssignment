...
public class CustomerController {
    ...
    public void runCustMainMenu() {
        ...
        do {
            ...
            switch (choice) {
                ...
                case 5:
                    clearScreen();
                    System.out.println("\n[View Booking History selected]\n");
                    viewBooking();
                    break;
                case 6:
                    clearScreen();
                    System.out.println("\n[Cancel Booking selected]\n");
                    cancelBooking(); // Add cancel booking option
                    break;
                case 7:
                    clearScreen();
                    System.out.println("\n[Check-in selected]\n");
    ...
    public void viewCustProfile() {
        ...
        while (true) {
            clearScreen();
            System.out.print("[Modify Profile selected]\n");
        ...
        while (true) {
            ...
            do {
                ...
                if (scanner.hasNextInt()) {
                    ...
                    if (choice >= 0 && choice <= 8) { // Ensure valid range
                        break;
                    } else {
            ...
            switch (choice) {
                ...
                case 6:
                    ...
                    viewRoomAvailable(); // Redirect to room availability
                    return; // Exit the method after viewing room availability
                case 7:
                    clearScreen();
                    System.out.println("\n[Viewing Room Types...]\n");
                    viewRoomType(); // Redirect to room types
                    return; // Exit the method after viewing room types
                case 8:
                    clearScreen();
                    System.out.println("\n[Returning to Customer Main Menu...]\n");
                    runCustMainMenu();
            ...
            runPaymentMethod(totalPrice, bookingId);
    ...
    public void cancelBooking() {
        System.out.println("Your Bookings for Cancellation:\n");
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = br.readLine()) != null) {
                String[] parts = line.split("\\|");
                if (parts.length == 9 && parts[1].equals(loggedInCustomer.getCustomerId()) && !parts[5].equals("CANCELED")) {
                    bookings.add(parts);
                    bookingCount++;
                    System.out.printf("[%d] Booking ID: %s | Room: %s | Room Number: %s | Status: %s | Check-In: %s | Check-Out: %s\n",
                            bookingCount, parts[0], parts[2], parts[8], parts[5], parts[6], parts[7]);
                }
            }
        } catch (IOException e) {
        ...
        int choice;
        while (true) {
            System.out.print("\nSelect a booking to cancel (1 - " + bookingCount + ", or '0' to cancel): ");
            if (scanner.hasNextInt()) {
                choice = scanner.nextInt();
                scanner.nextLine();
        ...
        if (choice == 0) {
            System.out.println("\n[Cancellation process aborted. Returning to Customer Main Menu.]\n");
            runCustMainMenu();
            return;
        }
    
        // Update the selected booking to CANCELED
        String[] selectedBooking = bookings.get(choice - 1);
        String roomNumber = selectedBooking[8]; // Room number
        selectedBooking[5] = "CANCELED"; // Update status to CANCELED
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                String[] parts = line.split("\\|");
                if (parts[0].equals(selectedBooking[0])) {
                    line = String.join("|", selectedBooking); // Update the booking line
                }
                newFileContent.append(line).append("\n");
            }
            br.close();
        ...
        // Update Room.txt to set the room status to "Available"
        try {
            StringBuilder newFileContent = new StringBuilder();
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (parts[0].equals(roomNumber)) {
                    parts[5] = "Available"; // Update room status to Available
    ...
    public void custCheckOut() {
        System.out.println("Your Bookings for Check-out:");
        File file = new File(BOOKINGS_FILE);
        if (!file.exists()) {
            System.out.println("\n[No bookings found]\n");
            showPostCheckoutOptions();
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(BOOKINGS_FILE))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (parts.length == 9) { // Ensure the line has exactly 9 parts
                    ...
                    // Check if the booking belongs to the logged-in customer and is checked in
                    if (bookingCustomerId.equals(loggedInCustomer.getCustomerId()) && status.equals("CHECKED_IN")) {
                        bookingCount++;
                        checkedInBookings.append(String.format("[%d] Booking ID: %s | Room: %s | Room Number: %s | Check-In: %s | Check-Out: %s\n",
    ...
    public void custFeedback(){
        clearScreen();
        System.out.println("\n=================================================");
        System.out.println("||                   Feedback                  ||");
        System.out.println("=================================================");
        System.out.println("Would you like to procide any feedbacks or complaints?");
        ...
        while(true){
            ...
            if(scanner.hasNextInt()){
                ...
                scanner.nextLine();
                if(feedbackChoice == 1 || feedbackChoice == 2){
                    break;
                }else{
    ...
    public void runPaymentMethod(double totalPrice, String bookingId) {
    ...
    switch (choice) {
        ...
        case 2: // Credit Card
            ...
            while (!cardNumber.matches("\\d{16}")) { // Validate card number (16 digits)
            ...
            while (!expiryDate.matches("(0[1-9]|1[0-2])/(\\d{2})")) { // Validate expiry date format
            ...
            while (!cvv.matches("\\d{3}")) { // Validate CVV (3 digits)
                System.out.println("[Invalid CVV. Please enter a valid 3-digit CVV.]");
                System.out.print("Enter CVV (3 digits): ");
                cvv = scanner.nextLine().trim();
            }

            payment = new CreditCardPayment(totalPrice, cardNumber, expiryDate, cardHolderName, cvv);
            break;
        case 3: // Debit Card
            clearScreen();
            ...
            payment = new DebitCardPayment(totalPrice, cardNumber, expiryDate, cardHolderName, cvv);
            break;
        case 0:
    ...
    if (payment != null) {
        ...
        try (BufferedWriter bw = new BufferedWriter(new FileWriter("transactions.txt", true))) {
            ...
            String transactionDetails = transactionId + "|" + loggedInCustomer.getCustomerId() + "|" + bookingId + "|" + payment.getPaymentMethod() + "|" + totalPrice + "|" + formattedDate;
        ...
        generateReceipt(transactionId, payment.getPaymentMethod(), totalPrice);
    ...
    // clear screen
    // print ASCII escape code for clear screen
