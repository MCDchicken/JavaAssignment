...
public class CustomerController {
    ...
    private void updateCustomerFile() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(CUSTOMER_FILE))) {
            ...
            while ((line = br.readLine()) != null) {
                String[] parts = line.split("\\|");
                if (parts[0].equals(loggedInCustomer.getUsername())) {
    ...
    public void custBookRoom() {
        ...
        System.out.println("\nAvailable Room Types (per nights):");
        ...
        switch (choice) {
            ...
            Booking booking = new Booking(bookingId, loggedInCustomer.getCustomerId(), roomType, nights, totalPrice, "PENDING");
    ...
    private String generateBookingId() {
        ...
        if (file.exists()) {
            try (BufferedReader br = new BufferedReader(new FileReader(file))) {
                ...
                while ((line = br.readLine()) != null) {
                    String[] parts = line.split("\\|");
                    if (parts.length > 0 && parts[0].startsWith("BKG")) {
                        try {
        ...
        return "BK" + id; // Prefix the ID with "BK"
    }
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(BOOKINGS_FILE))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (booking.getCustomerId().equals(loggedInCustomer.getCustomerId())) {
                    ...
                    System.out.println("Total Price: RM " + booking.getTotalPrice());
    ...
    public void custCheckIn(){
        ...
        if (choice == 0) {
            ...
            System.out.println("\n[Returning to Customer Main Menu...]\n");
            runCustMainMenu(); // Return to the customer main menu
        }
        return;
        }
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (firstComma != -1) {
                    ...
                    if (bookingUsername.equals(loggedInCustomer.getUsername())) {
                        ...
                        String totalPrice = line.substring(thirdComma + 1, fourthComma);
                        String status = line.substring(fourthComma + 1);
                        if (status.equals("PENDING")) {
                            bookingCount++;
                            pendingBookings.append("[" + bookingCount + "] Room: " + roomType + ", Nights: " + nights + ", Total Price: RM " + totalPrice + "\n");
                            //if (bookingCount == 1) {
                            //    selectedIndex = 0; // Default to first booking
                            //}
                        }
                    }
                }
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                String bookingUsername = line.substring(0, firstComma);
                if (bookingUsername.equals(loggedInCustomer.getUsername())) {
                    int fourthComma = line.lastIndexOf(",");
                    String status = line.substring(fourthComma + 1);
                    if (status.equals("PENDING")) {
                        if (currentIndex == targetIndex) {
                            line = line.substring(0, fourthComma + 1) + "CHECKED_IN";
                        }
                        currentIndex++;
                    }
                }
    ...
    public void custCheckOut(){
                ...
                if (firstComma != -1) {
                    ...
                    if (bookingUsername.equals(loggedInCustomer.getUsername())) {
                        ...
                        if (status.equals("CHECKED_IN")) {
                            bookingCount++;
                            checkedInBookings.append("[" + bookingCount + "] Room: " + roomType + ", Nights: " + nights + ", Total Price: RM " + totalPrice + "\n");
                            //if (bookingCount == 1) {
                            //    selectedIndex = 0;
                            //}
                        }
        ...
        int choice;
        while (true) {
            System.out.print("Select a booking to check out (1-" + bookingCount + ", or 0 to cancel): ");
            if (scanner.hasNextInt()) {
                choice = scanner.nextInt();
                scanner.nextLine();
                if (choice >= 0 && choice <= bookingCount) {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (bookingUsername.equals(loggedInCustomer.getUsername())) {
                    ...
                    if (status.equals("CHECKED_IN")) {
                        if (currentIndex == targetIndex) {
                            line = line.substring(0, fourthComma + 1) + "CHECKED_OUT";
                        }
                        currentIndex++;
                    }
                }
